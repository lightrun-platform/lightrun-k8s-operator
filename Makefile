...
export POST_COMMIT_HOOK
.PHONY: post-commit-hook
post-commit-hook:  ## Create git post-commit hook to generate crd.
	@echo "$$POST_COMMIT_HOOK" >> .git/hooks/post-commit
...
gocov: $(GOCOV) 
$(GOCOV): $(LOCALBIN)
	GOBIN=$(LOCALBIN) go install github.com/axw/gocov/gocov@latest

.PHONY: gocov-xml
gocov-xml: $(GOCOV_XML) 
$(GOCOV_XML): $(LOCALBIN)
	GOBIN=$(LOCALBIN) go install github.com/AlekSi/gocov-xml@latest
